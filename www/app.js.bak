...
async function deleteItem(name, type){
  ...
  try{
    ...
    if(type === 'directory'){
      await Filesystem.rmdir({ path, directory: Directory.ExternalStorage, recursive: true });
    }else{
...
async function loadPickerDir(path){
  ...
  try{
    const res = await Filesystem.readdir({
      path: path, directory: Directory.ExternalStorage
    });
    pickerPath = path;
...
window.addEventListener('load', function(){
  ...
  if(Capacitor && Capacitor.Plugins && Capacitor.Plugins.Filesystem){
    Capacitor.Plugins.Filesystem.checkPermissions().then(function(s){
