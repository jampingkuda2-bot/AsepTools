export function initCompress() {
  ...
  async function getFFmpeg(onLog) {
    ...
    progressLabel.textContent = "Mengunduh mesin proses video (±30MB, sekali aja — bisa 20-90 detik tergantung koneksi)...";
    ...
    const TIMEOUT_MS = 120000; // 2 menit — kalau lebih dari ini, kemungkinan besar koneksi/CDN bermasalah, bukan cuma lambat
    const withTimeout = (promise, label) => Promise.race([
      promise,
      new Promise((_, reject) => setTimeout(
        () => reject(new Error(`Gagal unduh ${label} — koneksi ke server pengunduh kelamaan (>2 menit). Cek internet kamu lalu coba lagi.`)),
        TIMEOUT_MS
      )),
    ]);

    const [coreURL, wasmURL, workerURL] = await Promise.all([
      withTimeout(toBlobURL(`${coreBaseURL}/ffmpeg-core.js`, "text/javascript"), "ffmpeg-core.js"),
      withTimeout(toBlobURL(`${coreBaseURL}/ffmpeg-core.wasm`, "application/wasm"), "ffmpeg-core.wasm"),
      ...
      withTimeout(toBlobURL(`${ffmpegBaseURL}/worker.js`, "text/javascript"), "worker.js"),
    ]);

    progressLabel.textContent = "Menyiapkan mesin proses video...";
    await withTimeout(ffmpeg.load({ coreURL, wasmURL, classWorkerURL: workerURL }), "mesin ffmpeg");
